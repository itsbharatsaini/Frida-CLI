...
def extract_documentation_python(information, one_function, funct_name):
    ...
    if one_function:
        ...
        code_pattern = re.compile(r"(?:\s*def\s*([\w\d_]*)\s*\(\s*(?:[\w,:\d_\s]*)\s*\)\s*(?:-\s*>\s*[\w.]*)*\s*:\s*(?:\"\"\"([\w\(\):.,`\s]*)\"\"\"))", re.DOTALL)
        ...
        for match in matches:
            ...
            if len(doc) >= 2:
                logger.info(__name__, f"More than 2...")
                for line in doc[1:]:
                    sep = line.split("\n")
                    if "Args:" in sep[0]:
                        logger.info(__name__, f"Args {sep[1:]}")
                        for arg in sep[1:]:
                            if "None" not in arg and arg.strip() != "":
                                logger.info(__name__, f"{arg}")
                                if first_parameter:
                                    lines.append(("bold", "Arguments:"))
                                    first_parameter = False
                                if ":" in arg:
                                    sep_arg = arg.strip().split(":")
                                    lines.append(("bullet", f"{sep_arg[0]}. {sep_arg[1]}"))
                                else:
                                    sep_arg = arg.strip()
                                    lines[-1][1] += sep_arg
                    if "Returns:" in sep[0]:
                        logger.info(__name__, f"Returns {sep[1:]}")
                        for ret in sep[1:]:
                            if "None" not in ret and ret.strip() != "":
                                logger.info(__name__, f"{ret}")
                                if first_return:
                                    lines.append(("bold", "Returns:"))
                                    first_return = False
                                if ":" in ret:
                                    sep_ret = ret.strip().split(":")
                                    lines.append(("bullet", f"{sep_ret[0]}. {sep_ret[1]}"))
                                else:
                                    sep_ret = ret.strip()
                                    lines[-1][1] += sep_ret
                    if "Raises:" in sep[0]:
                        logger.info(__name__, f"Raises {sep[1:]}")
                        for rai in sep[1:]:
                            if "None" not in rai and rai.strip() != "":
                                logger.info(__name__, f"{rai}")
                                if first_exception:
                                    lines.append(("bold", "Raises:"))
                                    first_exception = False
                                if ":" in rai:
                                    sep_rai = rai.strip().split(":")
                                    lines.append(("bullet", f"{sep_rai[0]}. {sep_rai[1]}"))
                                else:
                                    sep_rai = rai.strip()
                                    lines[-1][1] += sep_rai
            first_parameter = True
        ...
        for idx, line in enumerate(information["code"].splitlines(), 1):
            try:
                ...
                if param_match:
                    ...
                    lines.extend([("subheader", f"Function: {function_match.group(1)}")])
                    if information["description"].rstrip() != "":
                        lines.extend([("bold", "Description:"), ("text", information["description"].rstrip())])
